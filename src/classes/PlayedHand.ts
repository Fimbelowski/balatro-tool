...
export default class PlayedHand {
  constructor(public readonly cards: Card[]) {}

  public static containsFiveOfAKind(cards: Card[]) {
    const rankFrequencies = PlayedHand.getRankFrequencies(cards);

    return rankFrequencies.some((rankFrequency) => rankFrequency >= 5);
  }

  public static containsFlush(cards: Card[]) {
    const suitFrequencies = PlayedHand.getSuitFrequencies(cards);

    return suitFrequencies.some((value) => value >= 5);
  }

  public static containsFlushFive(cards: Card[]) {
    const cardsGroupedBySuit = PlayedHand.groupCardsBySuit(cards).filter(
      (group) => group.length >= 5,
    );

    return cardsGroupedBySuit.some((group) =>
      PlayedHand.containsFiveOfAKind(group),
    );
  }
  ...
  public static containsFlushHouse(cards: Card[]) {
    ...
    return cardsGroupedBySuit.some((group) =>
      PlayedHand.containsFullHouse(group),
    );
  }
  ...
  public static containsStraightFlush(cards: Card[]) {
    ...
    return cardsGroupedBySuit.some((group) =>
      PlayedHand.containsStraight(group),
    );
  }
  ...
  public get highestRankingPokerHand() {
    if (PlayedHand.containsFlushFive(this.cards)) {
      return PokerHand.FlushFive;
    }

    if (PlayedHand.containsFlushHouse(this.cards)) {
      return PokerHand.FlushHouse;
    }

    if (PlayedHand.containsFiveOfAKind(this.cards)) {
      return PokerHand.FiveOfAKind;
    }

    if (PlayedHand.containsStraightFlush(this.cards)) {
      return PokerHand.StraightFlush;
    }

    if (PlayedHand.containsFourOfAKind(this.cards)) {
      return PokerHand.FourOfAKind;
    }

    if (PlayedHand.containsFullHouse(this.cards)) {
      return PokerHand.FullHouse;
    }

    if (PlayedHand.containsFlush(this.cards)) {
      return PokerHand.Flush;
    }

    if (PlayedHand.containsStraight(this.cards)) {
      return PokerHand.Straight;
    }

    if (PlayedHand.containsThreeOfAKind(this.cards)) {
      return PokerHand.ThreeOfAKind;
    }

    if (PlayedHand.containsTwoPair(this.cards)) {
      return PokerHand.TwoPair;
    }

    if (PlayedHand.containsPair(this.cards)) {
